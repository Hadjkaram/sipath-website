...
export default function AdminDashboard() {
  ...
  // 2. Récupération dynamique des données à chaque changement d'onglet
  ...
  // 3. Fonction de SUPPRESSION (Générique pour toutes les tables)
  ...
  const handleAddActualite = async (e: React.FormEvent) => {
    ...
    const { error } = await supabase.from('actualites').insert([{
      titre: actTitre, categorie: actCategorie, date_pub: actDate, contenu: actContenu, image_url: imageUrl || null
    }]);
    if (!error) {
  ...
  return (
    <main className="min-h-screen bg-[#FAF5F0] flex flex-col md:flex-row">
      {/* Sidebar */}
      <aside className="w-full md:w-64 bg-[#2C2522] text-white flex flex-col justify-between shrink-0">
        <div className="p-8">
          ...
          <nav className="flex flex-col gap-5 text-xs tracking-wider uppercase font-semibold">
            <button onClick={() => { setActiveTab('annuaire'); setFeedback({type:'',message:''}); }} className={`text-left pb-2 border-b ${activeTab === 'annuaire' ? 'border-[#F26522] text-[#F26522]' : 'border-transparent text-white/50'}`}>Annuaire</button>
            <button onClick={() => { setActiveTab('equipe'); setFeedback({type:'',message:''}); }} className={`text-left pb-2 border-b ${activeTab === 'equipe' ? 'border-[#F26522] text-[#F26522]' : 'border-transparent text-white/50'}`}>Membres Bureau</button>
            <button onClick={() => { setActiveTab('actualites'); setFeedback({type:'',message:''}); }} className={`text-left pb-2 border-b ${activeTab === 'actualites' ? 'border-[#F26522] text-[#F26522]' : 'border-transparent text-white/50'}`}>Actualités</button>
          </nav>
        </div>
      ...
      <section className="flex-1 p-8 md:p-16 overflow-y-auto">
        <div className="max-w-3xl">
          {feedback.message && (
            <div className={`p-4 mb-8 text-xs font-bold tracking-wider uppercase border ${feedback.type === 'success' ? 'bg-green-50 text-green-700 border-green-200' : 'bg-red-50 text-red-600 border-red-200'}`}>{feedback.message}</div>
          )}

          {/* Formulaires d'ajout simplifiés pour lecture */}
          {activeTab === 'annuaire' && (
            <form onSubmit={handleAddAnnuaire} className="bg-white p-8 border border-[#E8D9C9]/60 shadow-sm flex flex-col gap-4 mb-12">
               ...
               <button type="submit" className="bg-[#2C2522] text-white py-4 uppercase text-xs hover:bg-[#F26522] mt-2">Enregistrer</button>
            </form>
          )}
          ...
          {activeTab === 'equipe' && (
            <form onSubmit={handleAddEquipe} className="bg-white p-8 border border-[#E8D9C9]/60 shadow-sm flex flex-col gap-4 mb-12">
               ...
               <input type="text" required value={eqTitle} onChange={(e) => setEqTitle(e.target.value)} placeholder="Titre (ex: Président)" className="bg-[#FAF5F0] p-4 text-sm" />
               ...
               <input type="text" value={eqSpecialties} onChange={(e) => setEqSpecialties(e.target.value)} placeholder="Spécialités (séparées par virgules)" className="bg-[#FAF5F0] p-4 text-sm" />
               ...
               <input type="file" onChange={(e) => setEqImageFile(e.target.files ? e.target.files[0] : null)} className="bg-[#FAF5F0] p-4 text-sm" />
               <button type="submit" className="bg-[#2C2522] text-white py-4 uppercase text-xs hover:bg-[#F26522] mt-2">Enregistrer</button>
            </form>
          )}
          ...
          {activeTab === 'actualites' && (
            <form onSubmit={handleAddActualite} className="bg-white p-8 border border-[#E8D9C9]/60 shadow-sm flex flex-col gap-4 mb-12">
               ...
               <input type="text" required value={actDate} onChange={(e) => setActDate(e.target.value)} placeholder="Date (ex: Août 2026)" className="bg-[#FAF5F0] p-4 text-sm" />
               <textarea required rows={4} value={actContenu} onChange={(e) => setActContenu(e.target.value)} placeholder="Contenu de l'article" className="bg-[#FAF5F0] p-4 text-sm" />
               <input type="file" onChange={(e) => setActImageFile(e.target.files ? e.target.files[0] : null)} className="bg-[#FAF5F0] p-4 text-sm" />
               <button type="submit" className="bg-[#2C2522] text-white py-4 uppercase text-xs hover:bg-[#F26522] mt-2">Publier</button>
            </form>
          )}

          {/* LISTE DES ÉLÉMENTS ENREGISTRÉS AVEC BOUTON SUPPRIMER */}
          <div className="bg-white p-8 border border-[#E8D9C9]/60 shadow-sm">
             ...
             <ul className="divide-y divide-[#E8D9C9]/40">
               {itemsList.length === 0 ? (
                 <p className="text-sm text-[#6B5B4F] py-4">Aucun enregistrement trouvé pour le moment.</p>
               ) : (
                 itemsList.map(item => (
                   <li key={item.id} className="py-4 flex justify-between items-center group">
                     <div>
                       <p className="text-sm font-bold text-[#2C2522]">{item.nom || item.name || item.titre}</p>
                       <p className="text-xs text-[#6B5B4F] mt-1">{item.specialite || item.title || item.categorie}</p>
                     </div>
                     <button 
                       onClick={() => handleDelete(item.id)} 
                       className="text-xs text-red-500 font-bold tracking-widest uppercase opacity-0 group-hover:opacity-100 transition-opacity px-4 py-2 hover:bg-red-50 rounded"
                     >
                       Supprimer
                     </button>
                   </li>
